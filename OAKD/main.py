...
class StereoSGBM:
    ...
    def create_disparity_map(self, left_img, right_img, is_rectify_enabled=True):
        ...
        padImg = np.zeros(
            shape=[left_img.shape[0], self.max_disparity], dtype=np.uint8)
        ...
        self.disparity = self.disparity[0:self.disparity.shape[0],
                                        self.max_disparity:self.disparity.shape[1]]
...
M_left = np.array(calibObj.getCameraIntrinsics(
    calibObj.getStereoLeftCameraId(), width, height))
M_right = np.array(calibObj.getCameraIntrinsics(
